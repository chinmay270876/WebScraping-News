...
def store_article(title, description, subheading, image, category, time):
    try:
        conn = mysql.connector.connect(**db_config)
        cursor = conn.cursor()
        cursor.execute("""
            INSERT IGNORE INTO trending (title, description, subheading, image, category, time)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (title, description, subheading, image, category, time))
        conn.commit()
    ...
    try:
        ...
        if title_tag and content_tags:
            ...
            if title and content:
                ...
                store_article(title, content, None, None, category, datetime.datetime.now())

        # Find and scrape linked articles
        for a_tag in soup.find_all('a', href=True):
            article_url = urljoin(url, a_tag['href'])
            if 'hindustantimes.com' in article_url:  # Restrict scraping to Hindustan Times domain
...
if __name__ == "__main__":
    create_table()
    start_url = "https://www.hindustantimes.com/"
    scrape(start_url)